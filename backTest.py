"""
represents a single unit of data from a market data source the stock ticker, timestamp, opening price and closing price
"""
...
class TickData:
    def __init__(self, symbol, timestamp, open_price=0, close_price=0, total_volume=0):
        self.symbol = symbol
...
class MarketData:
    ...
    def add_close_price(self, time, symbol, price, volume):
    ...
    def get_timestamp(self, symbol):
        return self.__recent_ticks__[symbol].tmestamp


# information from the host component of this class -- using pandas_datareader to retrieve data
class MarketDataSource:
    ...
    def market_simulation(self):
        ...
        for time, row in data.iterrows():
            self.md.add_close_price(time, self.ticker, row['Adj Close'], row['Volume'])
...
# order class represents a single order sent by the strategy server
# each order contains timestamp, symbol, quantity, price and size -- once order is filled
# can be adjusted for different order types - limit, stop, etc
class Order:
...
# position class keeps track of current market positions and account balance
# the debts and credits of position_value variable based on the purchase and selling of assets
class Position:
    def __init__(self):
        self.symbol = None
    ...
    def event_fill(self, timestamp, is_buy, qty, price):
        ...
        change_value = qty * price * (-1 if is_buy else 1)
        self.position_value += change_value
...
"""
strategy class is the base class for all other strategy implementations
event tick is called when new market tick data arrives
event order method is called when there are order updates
event position method is called when positions are updated
"""
...
class meanRevertingStrat(Strategy):
    ...
    def store_price(self, market_data):
        timestamp = market_data.get_timestamp(self.symbol)
        self.prices.loc[timestamp, 'Adj Close'] = market_data.get_close_price(self.symbol)
    ...
    def calculate_z_score(self):
        ...
        returns = self.prices['Adj Close'].pct_change().dropna()
...
class BackTest:
    def __init__(self, symbol, start, end, source='yahoo'):
    ...
    def updated_filled_positions(self, symbol, qty, is_buy, price, timestamp):
        position = self.get_position(symbol)
        position.event_fill(timestamp, is_buy, qty, price)
        self.strategy.event_position(timestamp, is_buy, qty, price)
    ...
    def is_order_unmatched(self, order, prices):
        ...
        if order.is_market_order and timestamp > order.timestamp:
            ...
            self.updated_filled_positions(symbol, order.qty, order.is_buy, open_price, timestamp)
    ...
    def start_backtest(self):
        self.strategy = meanRevertingStrat(self.target_symbol)
        ...
        mds.start, mds.end = self.start, self.end

        print("Backtesting started...")
        mds.market_simulation()
        print("Completed.")
